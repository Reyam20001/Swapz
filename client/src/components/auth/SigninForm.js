...
const SigninForm = () => 
{
  ...
  {
    try {
      const response = await fetch('http://localhost:5000/autherization/signin', 
      {
        method: 'POST',
        headers: {
  ...
  return (
    <form className="flex  flex-col p-10 gap-5 bg-gray-800 w-fit"
      ...
      <button
        type="submit" className="bg-white rounded-xl my-4 py-2 px-8 self-center">Sign in</button>
    </form>
  );
};
